...
sudo unzip -q -o /home/opc/backend.zip -d /var/www/safetywatch-api/
sudo chown -R nginx:nginx /var/www/safetywatch-api

cd /var/www/safetywatch-api

# The backend directory contains the whole laravel app
# Wait, did we zip "backend" or the contents of "backend"?
# We zipped "backend/*" into backend.zip, so the contents are directly in the zip root.
